 ...
 * opts.mubsubchannel: mubsub store. Required for multiple process functionality (e.g. cluster, load balanced servers)
 */
function AutoUri(hostname, opts) {
    ...
    // Mubsub allows load auto provisioned URIs to be called on other load balanced balanced servers or
    // processes. Owning process still manages app logic but process that received request will send back
    // response.
    self.mubsub = opts.mubsub || null;
    if (self.mubsub) {
        var Mubsub = require("mubsub");

        self.mubsub.collection = self.mubsub.collection || "autouri"; // capped collection name
        self.mubsub.size = self.mubsub.size || 100000 ; // max size in bytes for capped collection
        self.mubsub.num = self.mubsub.num || null ; // max number of documents inside of capped collection
        self.mubsub.url = self.mubsub.url || null ; // db url e.g. "mongodb://localhost:27017/yourdb"
        self.mubsub.connection = self.mubsub.connection || null; // db connection to use (in preference to url)
        var mdb = self.mubsub.connection || self.mubsub.url;

        self.mubsubclient = Mubsub(mdb, {safe: true});

        self.mubsubchannel = self.mubsubclient.channel(self.mubsub.collection, self.mubsub);
    }
    ...
    self.handleRequest = function(type) {
        return function(req, res) {
            ...
            if (!self.mubsubchannel) {
                    // Unprovisioned URI
                    return req.send(404);
                    // throw new Error('Unprovisioned uri requested: Unknown index.');
            }

            // Publish index on mubsub channel and wait for owning process to generate result
            ...
            subscription = self.mubsubchannel.subscribe(index + "response", function (doc) {
            ...
            self.mubsubchannel.publish(index + "request", {
                index: index,
                method: type,
                body: req.body
            });
...
AutoUri.prototype.addCallback = function(method, fn, opts) {
    ...
    if (self.mubsubchannel) {
        ...
        var subscription = self.mubsubchannel.subscribe(index + "request", function(doc) {
            ...
            res.send = function(body) {
                self.mubsubchannel.publish(index + "response", {
                    status: status,
                    body: body
                });
