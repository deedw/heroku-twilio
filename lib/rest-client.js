...
RestClient.prototype.getApplications = function(suc, err) {
    this.apiCall('GET', '/Applications', null, suc, err);
};

RestClient.prototype.updateApplicationInfo = function(params, suc, err) {
    this.apiCall('POST', '/Applications', {params: params}, suc, err);
};
...
// Country should be 2 letter ISO Country Code e.g. 'US'
// Possible filters: AreaCode, Contains, SmsEnabled, MmsEnabled, VoiceEnabled,
// ExcludeAllAddressRequired, ExcludeLocalAddressRequired, ExcludeForeignAddressRequired,
// Beta
// And for US and CA numbers:
// NearNumber, NearLatLong, Distance, InPostalCode, InRegion,
// InRateCenter, InLata
...
// Country should be 2 letter ISO Country Code e.g. 'US'
// Possible filters: Contains, SmsEnabled, MmsEnabled, VoiceEnabled,
// ExcludeAllAddressRequired, ExcludeLocalAddressRequired, ExcludeForeignAddressRequired,
// Beta
RestClient.prototype.getAvailableMobileNumbers = function(country, filters, suc, err) {
    if(!country) {
        throw new Error('Country argument required');
    }
    this.apiCall('GET', '/AvailablePhoneNumbers/' + country + '/Mobile',
                 {params: filters}, suc, err);
};
...
RestClient.prototype.getApplications = function(suc, err) {
    this.apiCall('GET', '/Applications', suc, err);
};

RestClient.prototype.createApplication = function(params, suc, err) {
    this.apiCall('POST', '/Applications', {params: params}, suc, err);
};

RestClient.prototype.updateApplication = function(sid, params, suc, err) {
    this.apiCall('POST', '/Applications/' + sid, {params:params}, suc, err);
};
...
RestClient.prototype.purchaseIncomingNumber = function( params, suc, err ) {
...
RestClient.prototype.getCallList = function(filters, suc, err) {
    this.apiCall('GET', '/Calls', {params: filters}, suc, err);
};
...
RestClient.prototype.getCallInstance = function(sid, suc, err) {
    ...
    this.apiCall('GET', '/Calls/' + sid, null, suc, err);
};
...
/**
 * getMessageInstance: Request a Message instance resource
 *
 * @param {String} sid: The sid of the message to retrieve.
 */
RestClient.prototype.getMessageInstance = function(messageSid, suc, err) {
    this.apiCall('GET', '/Messages/' + messageSid, null, suc, err);
};
RestClient.prototype.getSmsInstance = RestClient.prototype.getMessageInstance;

/**
 * getMessageList: Request a list of Message instance resources
 *
 * @param {Map} filters: An optional map of filters. See:
 * https://www.twilio.com/docs/api/rest/message
 */
RestClient.prototype.getMessageList = function(filters, suc, err) {
    this.apiCall('GET', '/Messages', {params: filters}, suc, err);
};
RestClient.prototype.getSmsList = RestClient.prototype.getMessageList;

/**
 * sendMessage: Send a new SMS Message
 *
 ...
 */
RestClient.prototype.sendMessage = function(from, to, body, uri, suc, err) {
    if(!from || !to || !body) {
    ...
    this.apiCall('POST', '/Messages', {params: params}, suc, err);
};
RestClient.prototype.sendSms = RestClient.prototype.sendMessage;
