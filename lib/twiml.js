...
var Num = module.exports.Num = function Num(body, attrs) {
    ...
    if(!attrs.url && global.autoUri) {
        var uri = global.autoUri.addCallback('POST', function(req, res) {
            var resp = new Response(res);
            var haslisteners = self.emit('callAnswered', req.body, resp);
            if (!haslisteners) resp.send(); //empty
        }, {maxRequests: 1});
        this.attrs.url = uri;
        this.attrs.method = 'POST';
    }
...
util.inherits(Num, EventEmitter);
...
var Redirect = module.exports.Redirect = function Redirect(uri, attrs) {
    ...
    if(!uri) {
        if (global.autoUri) {
            uri = global.autoUri.addCallback('POST', function(req, res) {
                self.emit('redirected', req.body, new Response(res));
            }, {maxRequests: 1});
        } else {
...
util.inherits(Redirect, EventEmitter);
